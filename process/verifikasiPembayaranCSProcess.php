<?php
    ...
    if(isset($_POST['verifikasiPembayaran'])){
        ...
        "UPDATE transaksi SET STATUS_TRANSAKSI='Selesai' WHERE ID_TRANSAKSI='$id' ")
        ...
        if($query){
            echo
                '<script>
                alert("Verifikasi Pembayaran Success"); window.location = "../page/listTransaksiCSPage.php"
                </script>';
        }else{
            echo
                '<script>
                alert("Verifikasi Pembayaran Failed");
                </script>';
        }
